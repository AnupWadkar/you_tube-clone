...
export default function HistoryContent() {
  ...
  const handleRemove = async (historyId: string) => {
    // Implement delete API call if available; for now just update local state
    // For demonstration, we remove from UI only.
    try {
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={() => handleRemove(item._id)}>
                  <X className="w-4 h-4 mr-2" /> Remove from watch history
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
